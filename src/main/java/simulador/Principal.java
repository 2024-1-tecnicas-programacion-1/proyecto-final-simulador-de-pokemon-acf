...
public class Principal {
    ...
    public static void main(String[] args) {
        ...
        LinkedList<Pokemon> listaPokemonesEntrenador = new LinkedList<>();
        Pokemon charmander= new Charmander("Charmander", 39, 52, TipoPokemon.Tipos.FUEGO, Estado.Estados.NORMAL);
        Pokemon Shellder= new Shellder("Shellder", 30, 65, TipoPokemon.Tipos.AGUA, Estado.Estados.NORMAL);
        Pokemon caterpie= new Caterpie("Caterpie", 45, 30, TipoPokemon.Tipos.BICHO, Estado.Estados.NORMAL);
        Pokemon jolteon = new Jolteon("Jolteon ", 65, 65, TipoPokemon.Tipos.ELECTRICO, Estado.Estados.NORMAL);
        Pokemon drowzee = new Drowzee("Drowzee", 60, 48, TipoPokemon.Tipos.PSIQUICO, Estado.Estados.NORMAL);
        Pokemon diglett= new Diglett("Diglett", 10, 55, TipoPokemon.Tipos.TIERRA, Estado.Estados.NORMAL);
        Pokemon articuno  = new Articuno("Articuno ", 90, 85, TipoPokemon.Tipos.HIELO.VOLADOR, Estado.Estados.NORMAL);
        Pokemon snorlax = new Snorlax("Snorlax", 160, 110, TipoPokemon.Tipos.NORMAL, Estado.Estados.NORMAL);
        Pokemon hitmonlee = new Hitmonlee("Hitmonlee", 50, 120, TipoPokemon.Tipos.LUCHA, Estado.Estados.NORMAL);
        Pokemon tentacool = new Tentacool("Tentacool", 40, 40, TipoPokemon.Tipos.AGUA.VENENO, Estado.Estados.NORMAL);
            ...
            case "1"://1. Gestionar Entrenadores
                ...
                switch (opcion) {
                    case "1"://1. Registrar nuevo entrenador
                        ...
                        Entrenador entrenador = new Entrenador(nombreEntrenador, listaPokemonesEntrenador);
                    ...
                    case "3"://3. Seleccionar un entrenador
                        ...
                        for (int i = 0; i < listaEntrenadores.size(); i++) {
                            if (nombreEntrenadorElegir == listaEntrenadores.get(i).getNombre()) {
            ...
            case "2"://"2. Gestionar Pokémones"
                ...
                switch (opcion) {
                    case "1"://1. Ver todos los Pokémones registrados
                        for (int i = 0; i < listaPokemonesEntrenador.size(); i++) {
                            System.out.println(listaPokemonesEntrenador.get(i).getNombre());
                        }
                        break;
                    case "2"://2. Registrar nuevo Pokémon
                        System.out.println("Los pokémones que puedes registrar son:");
                        for (int i = 0; i < listaPokemonesDisponibles.size(); i++) {
                            System.out.println(listaPokemonesDisponibles.get(i).getNombre());
                        }
                        System.out.println("Entonces, ¿que Pokémon quieres agregar a tu equipo?");
                        String nombrePokemonAgregar= sc.nextLine();
                        for (int i = 0; i < listapo; i++) {
